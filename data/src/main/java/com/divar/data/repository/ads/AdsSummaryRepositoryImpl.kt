...
class AdsSummaryRepositoryImpl @Inject constructor(
    ...
    override suspend fun getAdsSummary(page: Int): Flow<DataResult<Paging<List<AdsSummary>>>> = flow {
        safeCall { apiService.getAdsSummary(page = page) }
            .onSuccess { data ->
                val paging = data.toDomain(contentMapper = { it.map { adsSummaryResponse -> adsSummaryResponse.toDomain() } })
